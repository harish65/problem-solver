    ...
    @if($problem != null) 
    @if(($project->user_id == Auth::user()->id) || (!is_null($can_edit) && $can_edit->shared_with  == Auth::user()->id && $can_edit->editable_problem == 0))
            @include('adult.problem.problems_dd' , [$problems])
    @endif
    @if($can_edit !==null && $can_edit->shared_with == Auth::user()->id && $can_edit->editable_problem == 1)
        @php 
            $problem = \App\Models\Problem::where(['user_id' => Auth::user()->id , 'project_id' => $projectID ])->first();
        @endphp
    ...
    @else
        @if(($can_edit != null && $can_edit->editable_problem == 1) || ($can_edit == null && $project->user_id == Auth::user()->id))
